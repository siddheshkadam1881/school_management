...
module.exports = class AdminModel {
    ...
    async findStudent(form_data){
        var query = `SELECT id FROM mst_student where register_number=${form_data.register_number}`
        return await dbconnection.executevaluesquery(query, [form_data.register_number])
    }

    async updateFavouriteDetails(form_data) {
        var transactionQuery = ` update  customer_assign_offer set is_favourite= ?  where offer_code = ? and cms_customer_id = ? `
        return await dbconnection.executevaluesquery(transactionQuery, [form_data.is_fav, form_data.offer_code, form_data.cms_customer_id])
    }
